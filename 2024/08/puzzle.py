...
input : list[str]= [l.strip() for l in open("2024/08/input_test.txt").readlines()]
...
def part_1(grid: list[str]):
    ...
    for antenna_type in antenna_types:
        antenna_locations_antenna = [
            (y, x) for (y,x) in 
            get_antenna_locations(antenna_type, grid)
            if y < grid_y_size and y >= 0 and x < grid_x_size and x >= 0
        ]
        antinode_locations = antinode_locations.union(antenna_locations_antenna)
